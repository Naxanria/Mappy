...
//    MinecraftClient client = MinecraftClient.getInstance();
  ...
  {
    float dx = (float) Math.abs(wayPoint.pos.getX() - player.posX);
    float dy = (float) Math.abs(wayPoint.pos.getZ() - player.posZ);
...
//    }
    ...
    int size = 12;
    int x = (int) MathUtil.clamp(client.mainWindow.getScaledWidth() * scale - size / 2, 0, client.mainWindow.getScaledWidth() - size);
    int y = 4;
    ...
    drawStringCenteredBound(client.fontRenderer, dist + "m", x + size / 2, y, 0, client.mainWindow.getScaledWidth(), WHITE);
